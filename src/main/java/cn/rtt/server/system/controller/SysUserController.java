...
public class SysUserController {
    ...
    @PostMapping("/search")
    public Result<SysPage<SysUser>> search(@RequestBody UserSearchRequest request) {
        return Result.success(userService.search(request));
    }

    @PreAuthorize("@ss.hasPermission('system:user:list')")
    @PostMapping("/auth/search")
    public Result<SysPage<SysUser>> authSearch(@RequestBody UserSearchRequest request) {
        request.setStatus(UserStatus.OK.getCode());
        return Result.success(userService.search(request));
    }
